...
const SUBSET_EXPECTATIONS: CorpusExpectations = CorpusExpectations {
    ...
    inserted_projects: 12,
    inserted_source_files: 1_126,
    import_chunk_count: 98,
    project_count: 12,
    ...
    complete_chunk_count: 98,
    failed_chunk_count: 0,
    deferred_chunk_count: 98,
    startup_chunk_count: 0,
    chunk_action_rollup_count: 1_408,
    chunk_path_rollup_count: 20_795,
};
...
fn full_corpus_import_all_matches_expected_database_shape() -> Result<()> {
    ...
    assert_eq!(scan_report.inserted_projects, 37);
...
fn assert_subset_semantic_totals_from_signature(signature: &SubsetSemanticCounts) {
    ...
    assert_eq!(signature.source_files_with_scan_warnings_count, 16);
    assert_eq!(signature.git_project_count, 9);
    assert_eq!(signature.path_project_count, 3);
    assert_eq!(
